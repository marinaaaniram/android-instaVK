...
//        Log.i("VkWebViewClient ", "on destroy");
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            json = ParseJSON.getJsonFromUrl(urlString);
            Intent intentFilter = new Intent(REST_RESPONSE_BROADCAST);
            intentFilter.putExtra("responseJSON", json.getString("response"));
            intentFilter.putExtra("request_alias", intent.getStringExtra("request_alias"));
            sendBroadcast(intentFilter);

        } catch (JSONException e) {
            Log.i("JSON not valid", null);
        } catch (IOException e) {
            Log.i("Can not get response ", null);
        }
    }
}
